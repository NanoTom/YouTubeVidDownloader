...
def get_input():
    ...
    downloading_Thread = threading.Thread(target=download_video(user_input))
    #download_video(user_input)
...
def GetResolutions(yt):
    with lock:
        ...
        newWindowThread = threading.Thread(open_new_window(list))
...
root = tk.Tk()
root.title("Youtube Vid Downloader")

try:
    print("Trying")
    # Change the icon to a custom image file (replace "icon.png" with your image file)
    root.iconbitmap("nanotomnew.png")
except Exception as e:
...
#author info
author_info = tk.Label(root, text = "Made by Seif Elsayed",  fg = 'grey')
author_info.place(relx=0.85, rely=0.95, anchor='center',height=30)

#version info
version_info = tk.Label(root, text = "v1.0",  fg = 'grey')
version_info.place(relx=0.05, rely=0.95, anchor='center',height=30)
